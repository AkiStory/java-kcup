...
public class Stream_4 {
	static void main(String[] args) {
		...
		// 기준의 값이 key, 그 값에 해당하는 콜렉션이 value
		var groupa = names.stream().collect(Collectors.groupingBy(n -> n.length()));
		// 글자수 그룹 (3, 5, 7)
		System.out.println("group = " + groupa);

		var groupb = groupa.get(3); // a중 3만
		System.out.println("group(3) = " + groupb);
		...
		System.out.println(passedPerson); // true인 사람만 출력됌

		System.out.println("");
		//summarizingInt 배열의 요약본
		var summar = names.stream().collect(Collectors.summarizingInt(String::length));
		// var summar = names.stream().collect(Collectors.summarizingInt(name -> name.length()));
